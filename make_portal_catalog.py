...
tree = ET.parse('mars_events_portal.xml')
root = tree.getroot()
...
import os
if os.path.exists("PortalCatalog"):
  os.remove("PortalCatalog")

file1 = open("PortalCatalog","a")

print ('Name','Time','Quality','Type', sep='          ')
...
for e in range (0, len(xevents)):
    ...
    eventmars = eventpar[0].findall('mars:locationQuality',ns)
    if len(eventmars)>0:
        ...
        print (name, etime, mtype, mqual, sep=' ')

        line = [name,' ', etime,' ', mqual,' ', mtype + '\n']
